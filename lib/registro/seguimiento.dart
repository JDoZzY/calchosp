...
import 'dart:convert';
import '../ingresar/widgets/calendario_widget.dart';
import '../ingresar/widgets/input_widgets.dart';
import '../ingresar/widgets/servicio_widget.dart';
import '../registro/registro.dart';
import '../ingresar/date_input_formatter.dart';

import '../ingresar/laboratorios_section.dart';
...
class _SeguimientoScreen extends State<SeguimientoScreen> {
  ...
  void _guardarConsulta() {
    ...
    if (pesoController.text.isEmpty || tallaController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Peso y talla son campos requeridos')),
      );
      return;
    }
  ...
  void _cargarInitialData() {
    if (widget.initialData != null) {
      if (widget.initialData!['numeroRegistro'] != null) {
        registroController.text = widget.initialData!['numeroRegistro']?.toString() ?? '';
      }
      if (widget.initialData!['nombrePaciente'] != null) {
        nombreController.text = widget.initialData!['nombrePaciente']?.toString() ?? '';
      }
      if (widget.initialData!['numeroCama'] != null) {
        camaController.text = widget.initialData!['numeroCama']?.toString() ?? '';
      }
      camaController.text = widget.initialData!['numeroCama']?.toString() ?? '';
      registroController.text = widget.initialData!['numeroRegistro']?.toString() ?? '';
      servicioController.text = widget.initialData!['servicio']?.toString() ?? '';
      nombreController.text = widget.initialData!['nombrePaciente']?.toString() ?? '';
      fechaController.text = widget.initialData!['fechaNacimiento']?.toString() ?? '';
      edadController.text = widget.initialData!['edad']?.toString() ?? '';
      pesoController.text = widget.initialData!['peso']?.toString() ?? '';
      pesoReal = widget.initialData!['pesoReal'] as bool?;
      tallaController.text = widget.initialData!['talla']?.toString() ?? '';
      tallaReal = widget.initialData!['tallaReal'] as bool?;
      rodillaController.text = widget.initialData!['rodilla']?.toString() ?? '';
      cmbController.text = widget.initialData!['cmb']?.toString() ?? '';
      origenController.text = widget.initialData!['origen']?.toString() ?? '';
      diagnosticoNutriController.text = widget.initialData!['diagnosticoNutricional']?.toString() ?? '';
      if (diagnosticoNutriController.text.isEmpty && widget.initialData!['seguimientos'] != null && (widget.initialData!['seguimientos'] as List).isNotEmpty) {
        diagnosticoNutriController.text = (widget.initialData!['seguimientos'] as List).last['diagnosticoNutricional']?.toString() ?? '';
      }

      diagnosticoMedicoController.text = widget.initialData!['diagnosticoMedico']?.toString() ?? '';
      if (diagnosticoMedicoController.text.isEmpty && widget.initialData!['seguimientos'] != null && (widget.initialData!['seguimientos'] as List).isNotEmpty) {
        diagnosticoMedicoController.text = (widget.initialData!['seguimientos'] as List).last['diagnosticoMedico']?.toString() ?? '';
      }
    }
  }
                      ...
                      children: [
                        ...
                        Center(
                          child: ElevatedButton(
                            ...
                            child: const Text('Guardar Seguimiento', style: TextStyle(color: Colors.white)),
                          ),
                        ),
                        const SizedBox(height: 8),
                      ],
